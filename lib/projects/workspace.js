...
import { isProjectRoot, listProjectRoots, readProjectSummary, resolveCurrentProjectRoot } from "./discovery.js";
...
export async function setCurrentProject(workspaceRoot = process.cwd(), projectRootOrId) {
  ...
  let targetRoot = resolve(resolvedWorkspace, projectRootOrId);

  // When the project lives at the workspace root itself, its id equals
  // the workspace directory name.  resolve(workspace, id) would produce a
  // nested path that doesn't exist — fall back to the workspace root.
  if (!(await isProjectRoot(targetRoot)) && basename(resolvedWorkspace) === projectRootOrId) {
    if (await isProjectRoot(resolvedWorkspace)) {
      targetRoot = resolvedWorkspace;
    }
  }
