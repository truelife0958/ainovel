...
export async function readProjectSummary(projectRoot) {
  ...
  return {
    ...
    settingFilesCount: await countMarkdownFiles(join(resolvedRoot, "设定集")),
    outlineFilesCount: await countMarkdownFiles(join(resolvedRoot, "大纲")),
    chaptersCount: await countMarkdownFiles(join(resolvedRoot, "正文")),
  };
}
