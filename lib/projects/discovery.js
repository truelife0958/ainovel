...
/**
 * A directory that hosts the dev workspace (has package.json) is normally
 * NOT a writing project — even if a stale `.webnovel/state.json` lives there
 * from earlier test runs. Treating it as a project would pollute the source
 * tree (Tier 6 finding A: chapters saved into repo root `正文/`). Subdirs
 * are still allowed as candidates.
 */
async function isDevWorkspace(path) {
  return pathExists(join(path, "package.json"));
}
...
export async function listProjectRoots(workspaceRoot = process.cwd()) {
  ...
  const workspaceIsDev = await isDevWorkspace(resolvedRoot);
  const candidates = workspaceIsDev ? [] : [resolvedRoot];
...
export async function resolveCurrentProjectRoot(workspaceRoot = process.cwd()) {
  ...
  if (await pathExists(pointerPath)) {
    ...
    if (rawTarget) {
      ...
      // Refuse the dev workspace itself even if the pointer says so —
      // an accidental `cd` + relative pointer would otherwise treat the
      // repo as a project (Tier 6 finding A).
      const targetIsWorkspace = target === resolvedRoot && (await isDevWorkspace(target));
      if (!targetIsWorkspace && (await isProjectRoot(target))) {
        return target;
      }
    }
  }
