...
function safeChapterNumber(fileName) {
  try { return extractChapterNumber(fileName); }
...
function sortDocuments(kind, documents) {
  return documents.sort((left, right) => {
    if (kind === "chapter") {
      const chapterOrder = safeChapterNumber(left.fileName) - safeChapterNumber(right.fileName);
      if (chapterOrder !== 0) {
        return chapterOrder;
      }
