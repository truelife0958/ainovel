...
/**
 * Default abort-aware sleep. If `signal` fires during the wait, the
 * promise resolves immediately and the scheduler loop can break out
    ...
    while (true) {
      ...
      try {
        ...
        if (err && err.status === 429) {
          ...
          opts.onWait(sec);
          await sleep(sec * 1000, opts.signal);
          if (opts.signal?.aborted) return;
          continue;
        }
