...
def _resolve_workflow_project_root() -> Path:
    """Resolve project root while preserving legacy zero-arg call sites."""
...
def get_workflow_state_path() -> Path:
    ...
    project_root = _resolve_workflow_project_root()
...
def load_state():
    ...
    try:
        ...
        logger.warning("workflow_state.json corrupted, resetting: %s", exc)
        return {"current_task": None, "last_stable_state": None, "history": []}
