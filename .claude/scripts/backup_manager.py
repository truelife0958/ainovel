...
""")
    ...
    def rollback(self, chapter_num: int) -> bool:
        ...
        if status_output.strip():
            ...
            # 切换回默认分支（自动检测 main/master）
            default_branch = "main"
            for candidate in ["main", "master"]:
                check_ok, _ = self._run_git_command(["rev-parse", "--verify", candidate])
                if check_ok:
                    default_branch = candidate
                    break
            success, _ = self._run_git_command(["checkout", default_branch])
    ...
    def list_backups(self):
        ...
        for tag in tags:
            # 提取章节号（安全解析，跳过非数字tag如 "changelog"）
            try:
