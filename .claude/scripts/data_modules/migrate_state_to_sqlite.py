...
try:
    ...
    # 降级：如果 security_utils 不可用，提供简易原子写入
    import tempfile, os
...
def main():
    ...
    try:
        ...
        try:
            index_manager.log_tool_call(tool_name, False, error_code="MIGRATE_FAILED", error_message=str(exc))
        except Exception:
            pass
        raise SystemExit(1)
    ...
    if stats.get("errors", 0) > 0:
        ...
        try:
            index_manager.log_tool_call(tool_name, False, error_code="MIGRATE_ERRORS", error_message="迁移出现错误")
        except Exception:
            pass
        raise SystemExit(1)
    ...
    try:
        index_manager.log_tool_call(tool_name, True)
    except Exception:
        pass
