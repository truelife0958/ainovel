...
class DataModulesConfig:
    ...
    embed_api_key: str = field(default_factory=lambda: os.getenv("EMBED_API_KEY", ""), repr=False)
    ...
    rerank_api_key: str = field(default_factory=lambda: os.getenv("RERANK_API_KEY", ""), repr=False)
...
_config_created_at: float = 0
CONFIG_MAX_AGE_SECONDS = 600  # 10分钟后过期
import threading as _config_threading
_config_lock = _config_threading.Lock()


def get_config(project_root: Optional[Path] = None) -> DataModulesConfig:
    """获取配置，带自动过期机制（线程安全）"""
    global _default_config, _config_created_at
    import time
    ...
    now = time.time()
    if _default_config is None or (now - _config_created_at) > CONFIG_MAX_AGE_SECONDS:
        with _config_lock:
            # Double-check after acquiring lock
            now = time.time()
            if _default_config is None or (now - _config_created_at) > CONFIG_MAX_AGE_SECONDS:
                _default_config = DataModulesConfig()
                _config_created_at = now
