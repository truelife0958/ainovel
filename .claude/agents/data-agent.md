...
**review_score 校验**：review_score 必须为 0-100 的数值，非数值输入（null/undefined/字符串）视为 0 并记录警告。
...
**债务上限**：单笔债务 current_amount 不得超过 original_amount 的 10 倍。达到上限后停止计息并标记 status='capped'。
