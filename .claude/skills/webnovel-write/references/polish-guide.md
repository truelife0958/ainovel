...
**Anti-AI 重写熔断机制（防止无限循环）**：
- Anti-AI 检测→重写循环最多执行 **2 轮**
- 第 1 轮 fail → 聚焦 fail 原因重写，记录本轮改写点
- 第 2 轮仍 fail → **熔断退出**：
  - 将 `anti_ai_force_check` 标记为 `conditional_pass`
  - 在 deviation 中记录所有未通过项及保留理由
  - 使用 `AskUserQuestion` 询问用户：”Anti-AI 检测经过 2 轮重写仍有 N 处未通过，是否接受当前版本继续进入 Step 5？”
  - 用户确认后方可进入 Step 5，否则返回 Step 2A 全文重写
