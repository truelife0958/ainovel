...
- Step 1/5（可选，启用爆款机制提炼时）：[bestseller-mechanism-guide.md](../../references/bestseller-mechanism-guide.md)
...
If `idea_bank.json` contains `bestseller_mechanism_profile`, also load:
```bash
cat "${CLAUDE_PLUGIN_ROOT}/references/bestseller-mechanism-guide.md"
```
...
### 爆款机制继承策略（if `bestseller_mechanism_profile` exists）
- 仅继承机制，不继承具体剧情、角色骨架、核心装置或关键反转。
- 优先继承：
  - 开篇钩子组织方式
  - 爽点密度与兑现节奏
  - 升级/压制/反压制模式
  - 章末未闭合问题与钩子组合方式
  - 关系张力组织方式
- 若某一卷的核心冲突与参考作品明显相似，必须主动改写冲突源、代价机制或胜负条件。
...
**7. 爆款机制原创校验** (if `bestseller_mechanism_profile` exists)
- 是否只继承机制而未继承具体剧情？
- 是否避开了参考作品的角色关系骨架？
- 是否避开了参考作品的核心世界装置？
- 是否避开了参考作品的关键反转/高潮桥段/专属表达？
- 是否明确写出本章或本卷的差异化处理？
...
**4.5 爆款机制差异化检查** (if `bestseller_mechanism_profile` exists)
- 卷纲中已写出 `爆款机制继承`
- 卷纲中已写出 `原创差异化校验`
- 机制继承部分不包含具体剧情复用建议
- 差异化校验明确说明至少 2 个主动差异点
...
- 若启用爆款机制提炼，卷纲中已包含 `爆款机制继承` 与 `原创差异化校验`
...
- 启用 `bestseller_mechanism_profile` 后，卷纲缺少 `爆款机制继承` 或 `原创差异化校验`
- 爆款机制继承内容落入具体剧情、角色骨架或关键反转复用
