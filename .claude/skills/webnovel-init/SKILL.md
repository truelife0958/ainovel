...
- L2（按需）：
  ...
  - 爆款机制：`../../references/bestseller-mechanism-guide.md`
- L3（显式请求）：
...
- `references/system-data-flow.md`
  ...
  - 触发：Step 0 预检必读。
- `../../references/bestseller-mechanism-guide.md`
  - 用途：参考作品只提炼机制、不复用内容；统一爆款机制画像字段。
  - 触发：Step 4.5 用户提供参考作品或明确要求参考爆款时必读。
...
### Step 4.5：爆款机制提炼（可选但推荐）

目标：把参考热门作品转译为“机制层输入”，供后续创意约束与大纲规划继承，而不是做内容复用。

执行规则（硬约束）：
- 只允许提炼跨作品共性机制，不允许提炼具体桥段、人物、反转、名场面。
- 用户提供参考作品时，优先收集 `3-5` 本；若少于 `3` 本，允许继续，但必须标记“样本不足，稳定性较低”。
- 若用户不给参考作品，允许跳过，但必须显式标注“未启用爆款机制提炼”。
- 若分析结果出现具体剧情复用倾向，必须中断并回退到“机制层重写”。

收集项：
- 参考作品名（建议 `3-5` 本）
- 用户喜欢这些作品的原因
- 想借鉴的层级：开篇、升级、爽点、章末悬念、关系张力、群像、节奏

输出字段（写入 `idea_bank.json`）：
- `reference_titles`
- `market_mechanisms`
- `opening_hooks`
...
- 不直接复用核心世界装置
- 不直接复用关键反转、高潮桥段、结局兑现
- 不直接复用标志性台词、叙述包装或专属表达
- 若卷级核心冲突与参考作品高度相似，必须改写冲突源、代价机制、胜负条件至少一项

备注：
- 爆款机制提炼是创意约束的输入增强层，不能覆盖反套路与硬约束。
- 若用户明确要“像某一本”，必须改写为“提炼其吸引人的机制，并转译为原创方案”。
...
写入规则：
- 若用户启用爆款机制提炼，则必须写入 `bestseller_mechanism_profile`。
- 若未启用，则可不写该结构，或写入空结构并标注“未启用爆款机制提炼”。
- 该结构只记录机制层总结，不记录剧情复用建议。
...
- 若启用爆款机制提炼，`idea_bank.json` 中的 `bestseller_mechanism_profile` 已写入且仅包含机制层内容。
...
- 爆款机制提炼启用后，`bestseller_mechanism_profile` 缺失、字段名不一致或内容滑向剧情复用。
