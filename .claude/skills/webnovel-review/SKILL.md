...
推荐（bash）：
```bash
python "${SCRIPTS_DIR}/workflow_manager.py" start-task --command webnovel-review --chapter {end} || true
```
...
```bash
python "${SCRIPTS_DIR}/workflow_manager.py" start-step --step-id "Step 1" --step-name "加载参考" || true
python "${SCRIPTS_DIR}/workflow_manager.py" complete-step --step-id "Step 1" --artifacts '{"ok":true}' || true
```
...
```bash
python "${SCRIPTS_DIR}/update_state.py" --project-root "$PROJECT_ROOT" --add-review "{start}-{end}" "审查报告/第{start}-{end}章审查报告.md"
```
...
- 如用户明确授权可直接修改正文文件，则用 `Edit` 对对应章节文件做最小修复
- **Re-review 熔断**：修复后建议重新运行 `/webnovel-review` 验证，但同一章节的 review→fix→re-review 循环最多 **2 轮**
  - 第 1 轮修复后 re-review 仍有 critical → 允许第 2 轮修复
  - 第 2 轮 re-review 仍有 critical → **熔断退出**：输出”剩余 critical 问题需人工深度修改或通过 `/webnovel-write` 全文重写”，不再建议 re-review
...
```bash
python "${SCRIPTS_DIR}/workflow_manager.py" start-step --step-id "Step 8" --step-name "收尾" || true
python "${SCRIPTS_DIR}/workflow_manager.py" complete-step --step-id "Step 8" --artifacts '{"ok":true}' || true
python "${SCRIPTS_DIR}/workflow_manager.py" complete-task --artifacts '{"ok":true}' || true
```
